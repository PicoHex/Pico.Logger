...
结构化属性是 log event 本身的一部分，而不是另一种 logger 类型。`PicoLog.Abs` 现在是公开契约包，承载 logging 抽象、sink、formatter、`LogEntry` 与 flush 伴随契约；`PicoLog` 则提供运行时实现。
...
- **`PicoLog.Abs`**：公开契约包，包括 `ILogger`、`ILogger<T>`、`ILoggerFactory`、`LogLevel`、`LoggerExtensions`、`ILogSink`、`ILogFormatter`、`LogEntry`、`IFlushableLoggerFactory`、`IFlushableLogSink` 和 `FlushExtensions`
- **`PicoLog`**：运行时实现包，例如 `LoggerFactory`、`Logger<T>`、内置 sink、内置 formatter，以及异步管线 / 运行时行为
...
`ILoggerFactory.FlushAsync()` 扩展现在位于 `PicoLog.Abs`，作为公开契约包的一部分。严格的运行时能力仍由 `IFlushableLoggerFactory` 表达，而扩展方法让常见调用保持简单。
