...
結構化屬性是 log event 本身的一部分，而不是另一種 logger 類型。`PicoLog.Abs` 現在是公開契約套件，承載 logging 抽象、sink、formatter、`LogEntry` 與 flush 伴隨契約；`PicoLog` 則提供執行期實作。
...
- **`PicoLog.Abs`**：公開契約套件，包括 `ILogger`、`ILogger<T>`、`ILoggerFactory`、`LogLevel`、`LoggerExtensions`、`ILogSink`、`ILogFormatter`、`LogEntry`、`IFlushableLoggerFactory`、`IFlushableLogSink` 和 `FlushExtensions`
- **`PicoLog`**：執行期實作套件，例如 `LoggerFactory`、`Logger<T>`、內建 sink、內建 formatter，以及非同步管線 / 執行期行為
...
`ILoggerFactory.FlushAsync()` 擴充現在位於 `PicoLog.Abs`，作為公開契約套件的一部分。嚴格的執行期能力仍由 `IFlushableLoggerFactory` 表達，而擴充方法讓常見呼叫保持簡單。
