...
構造化プロパティは別の logger 型ではなく、ログイベントそのものの一部です。`PicoLog.Abs` は logging 抽象、sink、formatter、`LogEntry`、flush companion を含む公開コントラクトパッケージであり、`PicoLog` はランタイム実装を提供します。
...
- **`PicoLog.Abs`**: 公開コントラクトパッケージ。`ILogger`、`ILogger<T>`、`ILoggerFactory`、`LogLevel`、`LoggerExtensions`、`ILogSink`、`ILogFormatter`、`LogEntry`、`IFlushableLoggerFactory`、`IFlushableLogSink`、`FlushExtensions` を含む
- **`PicoLog`**: `LoggerFactory`、`Logger<T>`、組み込み sink、組み込み formatter、非同期パイプライン/ランタイム動作を含むランタイム実装パッケージ
...
`ILoggerFactory.FlushAsync()` 拡張は、公開コントラクトパッケージの一部として `PicoLog.Abs` にあります。厳密なランタイム機能は `IFlushableLoggerFactory` のままで、拡張は一般的な呼び出し位置をシンプルに保ちます。
