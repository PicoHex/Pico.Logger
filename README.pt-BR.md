...
As propriedades estruturadas fazem parte do próprio evento de log, não de um tipo de logger separado. `PicoLog.Abs` é o pacote de contrato público para abstrações de logging, sinks, formatters, `LogEntry` e companions de flush, enquanto `PicoLog` fornece a implementação de runtime.
...
- **`PicoLog.Abs`**: o pacote de contrato público, incluindo `ILogger`, `ILogger<T>`, `ILoggerFactory`, `LogLevel`, `LoggerExtensions`, `ILogSink`, `ILogFormatter`, `LogEntry`, `IFlushableLoggerFactory`, `IFlushableLogSink` e `FlushExtensions`
- **`PicoLog`**: pacote de implementação de runtime, como `LoggerFactory`, `Logger<T>`, sinks embutidos, formatters embutidos e o comportamento assíncrono de pipeline/runtime
...
A extensão `ILoggerFactory.FlushAsync()` fica em `PicoLog.Abs` como parte do pacote de contrato público. A capacidade estrita de runtime continua sendo `IFlushableLoggerFactory`, enquanto a extensão mantém simples o ponto de chamada comum.
